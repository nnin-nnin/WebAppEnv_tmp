# Nextcloud 29.0.1 Application Environment ()

这是 Nextcloud 29.0.1 的完整 Web Application Environment，targeting platform `linux/amd64`。最终交付是单镜像、单容器 all-in-one 镜像，容器内包含 Nextcloud、PHP 8.2、Apache 和 MariaDB 11.8.6；官方源码中的真实前端静态 bundle 也已包含在镜像内。
...
- Web Entrypoint: <http://127.0.0.1:18522/>。应显示 Nextcloud 真实登录页；登录后进入 Files 文件管理页面，可见应用导航和文件列表。
- 状态接口：<http://127.0.0.1:18522/status.php>。
- OCS API 根地址：<http://127.0.0.1:18522/ocs/v1.php/cloud/>。
- Initial Username: `admin`。
- Initial Role: administrator。
- Initial Password: `WcNext!26-cR7vK2P`
