...
HOST_PORT="${HOST_PORT:-18569}"
URL="http://127.0.0.1:${HOST_PORT}"

echo "=== Health Checking MantisBT at ${URL} ==="

cd "$APP_DIR"
docker compose -f docker/compose.yaml config --quiet

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${URL}/login_page.php" || true)
...
echo "=== MantisBT Health Check PASSED ==="
