...
- **目标应用**: Coppermine (v1.6.0)
- **目标目录**: `applications/coppermine/1.6.0/default`
- **验证结论**: **`PASSED`**
- **验证时间**: 2026-09-24T01:36:30+08:00
...
| 静态检查 | `static.scripts.syntax` | Shell 脚本权限与语法校验 (bash -n) | ✅ 通过 | 11.0ms | 全部 Shell 脚本均具备可执行权限且语法正确 |
...
| 动态检查 | `dynamic.runtime_verify` | 动态实机全流程验证 (up -> healthcheck -> login -> reset) | ✅ 通过 | 12.94s | 实机动态验证 PASS，端口 18632 正常工作 |
