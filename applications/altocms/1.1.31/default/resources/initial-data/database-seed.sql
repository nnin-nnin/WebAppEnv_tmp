CREATE DATABASE IF NOT EXISTS `altocms` CHARACTER SET utf8;
USE `altocms`;
...
INSERT INTO `prefix_page` (`page_id`, `page_pid`, `page_url`, `page_url_full`, `page_title`, `page_text`, `page_text_source`, `page_date_add`, `page_date_edit`, `page_seo_keywords`, `page_seo_description`, `page_active`, `page_main`, `page_sort`, `page_auto_br`) VALUES
(1, NULL, 'about', 'about', 'about', 'This is a test page', 'This is a test page', NOW(), NULL, '', '', 1, 1, 1, 1);
...
-- Update initial admin user credentials and role
UPDATE `prefix_user` SET
  `user_password` = '0x:9d29526f6c1f1ba4830350ca9c964c72',
  `user_mail` = 'admin@example.com',
  `user_activate` = 1,
  `user_role` = 3
WHERE `user_login` = 'admin';

INSERT INTO `prefix_user_administrator` (`user_id`)
SELECT `user_id` FROM `prefix_user` WHERE `user_login` = 'admin'
ON DUPLICATE KEY UPDATE `prefix_user_administrator`.`user_id` = `prefix_user_administrator`.`user_id`;

CREATE USER IF NOT EXISTS 'altocms'@'%' IDENTIFIED BY 'altocms_pass';
CREATE USER IF NOT EXISTS 'altocms'@'localhost' IDENTIFIED BY 'altocms_pass';
GRANT ALL PRIVILEGES ON `altocms`.* TO 'altocms'@'%';
GRANT ALL PRIVILEGES ON `altocms`.* TO 'altocms'@'localhost';
FLUSH PRIVILEGES;
