# GLPI 10.0.26 Application Environment ()

本目录交付 GLPI 10.0.26 的完整 Web Application Environment，固定源码 commit 为 `17b6d8de504deb8fa14825a86824f0f1cac9e10d`，targeting platform `linux/amd64`。最终产物是单镜像、单容器 all-in-one 环境：镜像内包含 GLPI PHP 应用、Apache、前端静态资源、Composer 依赖、MariaDB 和自动初始化逻辑。
...
```bash
cd applications/glpi/10.0.26
docker run -d -p 18514:80 yorem/glpi:10.0.26
```
