...
HOST_PORT="${HOST_PORT:-18009}"
URL="http://127.0.0.1:${HOST_PORT}"

echo "=== Health Checking AnchorCMS at ${URL} ==="

cd "$APP_DIR"
docker compose -f docker/compose.yaml config --quiet

HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${URL}/index.php/admin/login" || true)

if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 302 ]; then
    echo "SUCCESS: Admin login page returned HTTP ${HTTP_CODE}!"
else
...
echo "=== AnchorCMS Health Check PASSED ==="
