...
echo "=== Starting Review Board Compose Environment ==="
cd "${APP_DIR}"
...
while [ ${RETRY_COUNT} -lt ${MAX_RETRIES} ]; do
    UNHEALTHY=$(docker compose -f docker/compose.yaml ps --format json | grep -iE "starting|unhealthy|restarting" || true)
