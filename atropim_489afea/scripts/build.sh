...
archive_path="$image_dir/atropim-489afea-linux-amd64.tar"
...
docker build --platform linux/amd64 --pull=false \
  ...
  -t atropim:489afea "$project_dir"

docker save --output "$archive_path" atropim:489afea
docker image inspect atropim:489afea > "$metadata_path"
sha256sum "$archive_path" "$metadata_path" \
  | sed "s#  $image_dir/#  #" > "$checksum_path"

echo "AtroPIM all-in-one image exported: $archive_path"
